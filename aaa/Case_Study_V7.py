...
Visayas = "Visayas"
Mindanao = "Mindanao"
region = (Metro_Manila, Luzon, Visayas, Mindanao)
...
def region():
    ...
    region = (Metro_Manila, Luzon, Visayas, Mindanao)
    region_price = (110, 130, 180, 200)
    place = True
    while place:
        all_region =(input("What is your region?: "))
        while all_region != "0" and all_region != "1" and all_region != "2" and all_region != "3":
            print("")
            print("Invalid option.")
            print("")
            all_region =(input("What is your region?: "))
...
def total():
    if price == []:
        ...
        total = sum(price)
        VAT_tax = region()
        new_total = VAT_tax + total
        #Email
        email = True
        while email:
            valid = 0
            while valid == 0:
                emailadd = (input("Input your email: "))
                import re
                pat = "^[a-zA-Z0-9-.-_]+@[a-zA-Z0-9]+\.[a-z]{1,3}$"
                if re.match(pat,emailadd):
                    valid = 1
                else:
                    valid = 0
                    print ("Invalid Email Address.")
            email = False
        #Expected Recieval Date
        if VAT_tax == 110:
            expdate = "3-4 Days."
        elif VAT_tax == 130:
            expdate = "4-5 Days."
        ...
        #Output
        print("\n\t==RECEIPT==")
        print("Your total is: P",new_total)
        print("Expect your order in", expdate)
